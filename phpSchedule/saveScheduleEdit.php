...
if (strlen($periodYear)==2) $periodYear='20'.$periodYear;
  ...
  $fp = fopen('data/schedules/'.$scheduleName, "w");
 ...
 for($i=0; $i<$timeCount; $i++){
    ...
    fwrite($fp, $time[$i][0]); //Employee name
    fwrite($fp, $item1."\n"); //Times
  	fwrite($fp, $item2."\n"); //Departments
  ...
  $message="Revised Schedule Saved";
}

header('Location: editSchedule.php?message='.$message.' - '.date('l jS \of F Y h:i:s A'));
//die();
?>