<?php
	...
	*/
	...
	$sql = 'SELECT `items`.`item_ID`, `items`.`item_desc`, `items`.`item_retail`, 
	        ...
	        WHERE `departments`.`dept_belongs_to`='.$dept.' AND `item_pic` IS NOT NULL AND `vendor_ID` <> 1483 
	        OR `departments`.`dept_ID`='.$dept.' AND `item_pic` IS NOT NULL AND `vendor_ID` <> 1483';
	$result = mysqli_query($db, $sql);      
	mysqli_close($db);
  ...
  gtag('config', 'UA-8450012-2');
    ...
    <?php include 'z_banner.php'; ?>
        ...
        <div class="pagecontainer">
			...
			<div class="navbar">
			    <?php if($nav) { ?>
			    <a class="navbtn" href="shopItems.php?dept=<?= $dept ?>&keyName=<?= $keyName ?>&page=1" ><i class="fa fa-angle-double-left"></i>...</a>
			    <?php } ?>
			    ...
			    ?>
                <a class="<?= $class ?>" href="shopItems.php?dept=<?= $dept ?>&keyName=<?= $keyName ?>&page=<?= ($i+1) ?>"><?= ($i + 1) ?></a>
                <?php } ?>
                <?php if($pages > $TABS && $i < $pages ) { ?>
                <a class="navbtn" href="shopItems.php?dept=<?= $dept ?>&keyName=<?= $keyName ?>&page=<?= $pages ?>" >...<?= $pages ?></a>
                <?php } ?>                
            </div>
