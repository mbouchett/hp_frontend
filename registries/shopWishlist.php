<?php
	/* Homeport Website Front Page
		...
		http://www.homeportonline.com/shopWishList.php
	*/
	...
	$loggedIn = 0;
	$cartCount =  0;
	@$wishCust = safeString($_REQUEST['cust']);
	...
	$sql  = 'SELECT *  FROM `web_cust` WHERE `wc_ID` = '.$wishCust;
	...
	$sql = 'SELECT `items`.`item_ID`, `items`.`item_desc`, `items`.`item_retail`, `items`.`item_pic`,                                  `items`.`item_qty`, 
	        ...
	        WHERE `wc_ID`='.$wishCust.' AND `wish_fund` > 0 
	...
	for($i=0; $i < $fundCount; $i++){
		...
		$fund[$i]['balance'] = $fund[$i]['item_retail'] - $fund[$i]['wish_fundAmt'];
		$fund[$i]['fundPercent'] = ceil(($fund[$i]['wish_fundAmt']/$fund[$i]['item_retail']) * 100);
	}
	
	//Get The remainder of the wish items
	$db= new mysqli('localhost', $db_user, $db_pw, $db_db);
	$sql = 'SELECT `items`.`item_ID`, `items`.`item_desc`, `items`.`item_retail`, `items`.`item_pic`,                                  `items`.`item_qty`, 
	               `web_wish`.`wish_ID`,`web_wish`.`wish_qty`, `wish_recQty`, `web_wish`.`wish_fund`, `web_wish`.`wish_fundAmt` 
	        FROM `web_wish` 
	        LEFT JOIN `items` USING (`item_ID`) 
	        WHERE `wc_ID`='.$wishCust.' AND `wish_fund` = 0
	        ORDER BY `wish_ID` DESC';
...
<html>
	<head>
		<title>Homeport - Shop Wish List</title>
        ...
        <script type="text/javascript" src="js/shopWishlist.js" ></script>
	</head>
	<body>
    ...
    <?php include '../z_sub/z_banner.php'; ?>
        ...
        <div class="pagecontainer">
            <div class="navbar">
                Viewing Wishlist Items for &quot;<?= $wc['wc_fname'] ?>&quot;
            </div>
            ...
            <div class="funditemcontainer">
                ...
                <div class="options">
                    ...
                    <a href="../cart/processAddToCart.php?item=<?= $fund[$i]['item_ID'] ?>&amt=<?= $fund[$i]['balance'] ?>&wish=<?= $fund[$i]['wish_ID'] ?>&wcust=<?= $wishCust ?>" class="fundbtn">Pay Off Balance</a>
                    <input type="text" placeholder="Or Pitch In!" name="pitchIn">
                    <a class="fundbtn" onclick="contribute(<?= $fund[$i]['item_ID'] ?>, <?= $fund[$i]['wish_ID'] ?>, <?= $i ?>, <?= $wishCust ?>)">Contribute</a>
                    <?php }else { ?>
			...
			<div class="navbar">
			    <?php if($nav) { ?>
			    <a class="navbtn" href="shopWishlist.php?cust=<?= $wishCust ?>&page=1" ><i class="fa fa-angle-double-left"></i>...</a>
			    <?php } ?>
			    ...
			    ?>
                <a class="<?= $class ?>" href="shopWishlist.php?cust=<?= $wishCust ?>&page=<?= ($i+1) ?>"><?= ($i + 1) ?></a>
                <?php } ?>
                <?php if($pages > $TABS && $i < $pages ) { ?>
                <a class="navbtn" href="shopWishlist.php?cust=<?= $wishCust ?>&page=<?= $pages ?>" >...<?= $pages ?></a>
                <?php } ?>                
            </div>
            ...
            <div class="itemcontainer">
                ...
                <div class="witem">
                    ...
                    <?php if(($item[$i]['wish_qty'] - $item[$i]['wish_recQty']) > 0) { ?>
                    <div class="itemquans">
                        <div class="quandesired bold">QTY: <?= $item[$i]['wish_qty']-$item[$i]['wish_recQty'] ?></div>
						<?php if(($item[$i]['wish_qty'] - $item[$i]['wish_recQty']) > 1) { ?>                     
                    ...
                    <div class="itembtns">
                        <a onclick="addToCart(<?= $item[$i]['item_ID'] ?>, <?= $item[$i]['wish_ID'] ?>, <?= $i ?>, <?= $wishCust ?>)" class="addtocartbtn"><i class="fa fa-gift"></i>&nbsp;Add to Cart</a><!--visibility:hidden should be toggled based on quantaties and text field-->
                    </div>
                    <?php }else { ?>
                    <div class="itemquans">
